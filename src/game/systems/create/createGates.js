...
// overlap pra cada porta. Quando o player passa por ela, libera a "key"
// (mapKey vizinho) declarada na propriedade customizada "key" do objeto no
// Tiled.
export function createGates(scene) {
  ...
  gateLayer.objects.forEach((objectData) => {
    const mapKeyToUnlock = getTiledProperty(objectData.properties, 'key');
    if (!mapKeyToUnlock) return;
