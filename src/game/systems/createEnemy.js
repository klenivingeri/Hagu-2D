export function createEnemy(scene) {
    ...
    scene.inimigoOverlap = scene.physics.add.overlap(scene.bullets, enemy, destruirInimigo, null, scene);
    
    return enemy;
}

function destruirInimigo(bullet, inimigo) {
// Desativa o tiro
  ...
  // ** EM VEZ DE DESTRUIR, APENAS DESATIVA O INIMIGO **
  inimigo.setActive(false);
  inimigo.setVisible(false);
  inimigo.body.enable = false; // Desliga a física para ele não colidir mais
  inimigo.destroy();
}
